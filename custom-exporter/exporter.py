...
process_name_gauge = Gauge('process_name_info', 'Information about process',
                           ["process_name", "version", "up_time", "field1_name", "field1_value", "field2_name", "field2_value"])
...
def fetch_mysql_data():
    ...
    # 执行查询
    cursor.execute("SELECT process_name, version, up_time, field1_name, field1_value, field2_name, field2_value FROM prometheus")
    ...
    for row in rows:
        ...
        process_name_gauge.labels(
            ...
            field1_value=row['field1_value'],
            field2_name=row['field2_name'],
            field2_value=row['field2_value']
        ).set(1)
...
if __name__ == '__main__':
    # 启动一个 HTTP 服务来暴露指标
    start_http_server(8000)  # Prometheus 将通过 8000 端口抓取数据
